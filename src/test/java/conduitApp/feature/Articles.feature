...
Feature: Articles
  ...
  Scenario: Favorite/ Unfavorite the first article
    ...
    * def firstArticleId = response.articles[0].slug
    ...
    Given path 'articles', firstArticleId
    When method Get
    Then status 200
    * def initialFavoritesCount = response.article.favoritesCount

    Given path 'articles', firstArticleId, 'favorite'
    And request favoriteArticleBody
    When method Post
    Then status 200
    * def increasedCount = response.article.favoritesCount
    And match increasedCount == initialFavoritesCount + 1

    Given path 'articles', firstArticleId, 'favorite'
    When method Delete
    Then status 200
    * def decreasedCount = response.article.favoritesCount
    And match decreasedCount == increasedCount -1
    And match decreasedCount == initialFavoritesCount
